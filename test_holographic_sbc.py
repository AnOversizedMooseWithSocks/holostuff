...
import numpy as np
from holographic_sbc import (sbc_random, sbc_codebook, sbc_bind, sbc_unbind,
                             sbc_reconstruct, sbc_resonator)
...
def test_decompose_detects_absent_factor():
    from holographic_sbc import decompose_structure, sbc_identity
    B, L = 16, 16
...
def test_factors_cannot_be_read_off_a_product_naively():
    ...
    from holographic_sbc import sbc_onehot, decompose_structure
    B, L = 16, 16
    cbs = [sbc_codebook(B, L, 10, seed=k) for k in range(3)]
    true = (2, 5, 8)
