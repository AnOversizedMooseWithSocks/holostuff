...
### From the command line
    ...
    pytest -q                         # the whole test suite (2083 tests)
...
The engine (pure numpy):
    ...
    holographic_mesh.py       explicit polygon mesh: half-edge adjacency, Euler/genus, normals, OBJ + glTF buffers
    holographic_gltf.py       glTF 2.0 (.glb) emit/parse -- the binary boundary to a three.js front end
...
Tests (2083 total):
    ...
    test_holographic_planshape.py schema-guided typed plans/records + descend (IFMATCH generalized, abstains)
    test_holographic_graphsignal.py Taubin graph-Laplacian codebook denoise over its k-NN graph (RT-III1; no-shrink, high-noise win, low-noise kept negative)
    test_holographic_chart.py    nonlinear manifold chart -- Isomap/Laplacian-Eigenmaps beat linear SVD on a curved manifold (RT-II1; geodesic fidelity, deterministic)
    test_holographic_determinism.py the ISA determinism contract (ISA-1) -- one sign/tie-break rule, four scattered copies de-siloed onto it bit-exactly
    test_isa_conformance.py      the ISA conformance suite (ISA-2) -- every base op vs its definitional reference (TOL/EXACT), golden convolution identities, the bind_batch-class regression
    test_isa_extensions.py       the governed bind-mode extensions (ISA-3) -- Clifford exact rotation, FPE continuous kernel, tensor capacity; each earns its place by a measured regime win
    test_holographic_mesh.py     explicit polygon kernel (FWD-1) -- Euler/genus invariants, half-edge reciprocity, outward unit normals, OBJ + buffer round-trips, non-manifold rejection (kept negative: Python-loop bound, won't scale to interactive million-poly edits)
    test_holographic_gltf.py     glTF .glb boundary (FWD-2) -- structural validity, 4-byte alignment, position/normal/uv round-trips, byte-reproducible output, three.js-loadable (verified against pygltflib offline)
    test_isa_registers.py        the HoloMachine register file (ISA-4) -- exact STORE/RECALL named slots, the re-derivation saving, and the bundled-file capacity-cliff kept negative
    test_isa_callstack.py        the calling convention + permute-stack (ISA-5) -- frame-local registers/stack (the ABI), reverse-via-stack, and the stack's crosstalk depth cliff
    test_holographic_template.py the macro layer (ISA-6) -- parameterized recipe templates with holes, bit-exact distinct instantiations, a starter library, and the macro-hygiene kept negative
    test_holographic_lang.py     the structure language (ISA-7) -- an S-expr surface lowering to the recipe IR, round-trip, bit-exact realization, template agreement, and the one-domain scope boundary
    test_holographic_reversible.py the reversible/error-correction model (ISA-8) -- the reversibility audit, the oracle-free health signal, and the auto-cleanup scheduler beating a fixed cadence at matched fidelity
    test_holographic_steering.py the anisotropic/steering kernel (RT-IV1) -- FPE per-axis bandwidth beating the isotropic RBF on dense directional data, with the sparse/isotropic kept negatives
    test_holographic_iterate.py  spectral iteration (RT-I1) -- a bind operator's free FFT eigendecomposition: one-eval k-step jump matching k binds, closed-form limit, and convergence read off the spectrum
